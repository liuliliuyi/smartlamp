			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						String	url = UrlUtil.getUserUrl() + "blade-sun/api/device/selectDeviceSnByScanUserId";		
			...
			public void run() {
				...
				try {
					...
					String url = UrlUtil.getUserUrl()
							+ "blade-sun/api/device/scanDevice";
...
//		Toast.LENGTH_SHORT).show();
		...
		if(qq1.size()>1){
			for (int i = 0; i < (qq1.size()) * 0.5; i++) {
				...
				final String getsntext = replace(qq1.get(i * 2).toString()
						.replaceAll(" ", "").substring(8))
						+ replace(qq1.get((i * 2) + 1).toString().replaceAll(" ", "")
								.substring(8));
...
//		loading_view.setVisibility(View.GONE);
	...
	private void getGoodsList2(final String qrcode) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					String url = UrlUtil.getUserUrl()
							+ "blade-sun/api/device/checkTelecomNumber?sn="
							+ qrcode;
					SharedPreferences sp = getSharedPreferences("mrsoft",
							MODE_PRIVATE);
					String token = sp.getString("token", "token");
	...
	public class MainAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			double dev=(double) Math.round(Double.parseDouble(dianya)*100) / 100; 
			vh.guangbandianya.setText("光板电压:"+String.valueOf(dev));
